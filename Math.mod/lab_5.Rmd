...
ggplot(
  ...
  geom_jitter(position = position_jitter(width = .1, height = 0),
  pch = 21, col = rgb(0, 0, 1, alpha = 0.4), bg = rgb(0,0,1,alpha = 0.4),
  )
...
attach(DF.Boston)
# подгонка модели на обучающей выборке
fit.lm.2 <- lm(crim ~ poly(zn + rm, 2), subset = inTrain)
# прогноз на тестовую
...
attach(DF.Boston)
# подгонка модели на обучающей выборке
fit.lm.3 <- lm(crim ~ poly(zn + rm, 3), subset = inTrain)
# прогноз на тестовую
y.test.lm.3 <- predict(fit.lm.2, DF.Boston[-inTrain, ])
# считаем MSE на тестовой выборке
MSE.lm.3 <- round(mean((y.test.fact - y.test.lm.2)^2), 2)
detach(DF.Boston)
MSE.lm.3
```
...
for (i in 1:5) {
  # оценка модели
  fit.glm <- glm(crim ~ poly(zn + rm, i), data = DF.Boston)
  # расчёт ошибки
...
boot.fn.2 <- function(data, index){
  coef(lm(crim ~ poly(rm + zn, 2), data = data, subset = index))
}
